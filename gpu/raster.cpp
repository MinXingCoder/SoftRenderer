...
void Raster::rasterizeLine(std::vector<VsOutput>& resuts, const VsOutput& v0, const VsOutput& v1)
{
...
void Raster::interpolantLine(const VsOutput& v0, const VsOutput& v1, VsOutput& target)
{
    ...
    target.mColor = Lerp(v0.mColor, v1.mColor, weight);
    target.mUV = Lerp(v0.mUV, v1.mUV, weight);
}
...
void Raster::rasterizeTriangle(std::vector<VsOutput>& results, const VsOutput& v0, const VsOutput& v1, const VsOutput& v2)
{
    ...
    for(int32_t i = minX; i <= maxX; ++i)
    {
        ...
        {
            glm::i32vec2 p0(v0.mPosition.x - i, v0.mPosition.y - j),
                         p1(v1.mPosition.x - i, v1.mPosition.y - j),
                         p2(v2.mPosition.x - i, v2.mPosition.y - j);

            float r0 = Cross(p0, p1);
            float r1 = Cross(p1, p2);
            float r2 = Cross(p2, p0);
...
{
    glm::vec2 ab(v1.mPosition.x - v0.mPosition.x, v1.mPosition.y - v0.mPosition.y),
              ac(v2.mPosition.x - v0.mPosition.x, v2.mPosition.y - v0.mPosition.y);
    float abc = std::abs(Cross(ab, ac));
    glm::vec2 pb(v1.mPosition.x - target.mPosition.x, v1.mPosition.y - target.mPosition.y),
              pc(v2.mPosition.x - target.mPosition.x, v2.mPosition.y - target.mPosition.y);
    float bpc = std::abs(Cross(pb, pc));
    float alpha = bpc / abc;

    glm::vec2 pa(v0.mPosition.x - target.mPosition.x, v0.mPosition.y - target.mPosition.y);
    float apc = std::abs(Cross(pa, pc));
    float beta = apc / abc;

    float apb = std::abs(Cross(pa, pb));
    float gamma = apb / abc;

    target.mColor = Lerp(v0.mColor, v1.mColor, v2.mColor, alpha, beta, gamma);
